...
func exprParserInit() {
	...
	staticData.serializedATN = []int32{
		...
		6, 7, 1, 0, 4, 5, 27, 0, 4, 1, 0, 0, 0, 2, 13, 1, 0, 0, 0, 4, 5, 3, 2,
		1, 0, 5, 6, 5, 0, 0, 1, 6, 1, 1, 0, 0, 0, 7, 8, 6, 1, -1, 0, 8, 14, 5,
		3, 0, 0, 9, 10, 5, 1, 0, 0, 10, 11, 3, 2, 1, 0, 11, 12, 5, 2, 0, 0, 12,
...
func (s *MulDivContext) GetMulOp() antlr.Token { return s.mulOp }

func (s *MulDivContext) SetMulOp(v antlr.Token) { s.mulOp = v }
...
func (s *MulDivContext) MULTIPLY() antlr.TerminalNode {
	return s.GetToken(ExprParserMULTIPLY, 0)
}

func (s *MulDivContext) DIVIDE() antlr.TerminalNode {
...
func (s *AddSubContext) GetAddOp() antlr.Token { return s.addOp }

func (s *AddSubContext) SetAddOp(v antlr.Token) { s.addOp = v }
		...
		if _alt == 1 {
			...
			case 1:
				...
				}
				{
					...
					localctx.(*MulDivContext).mulOp = _lt
					...
					if !(_la == ExprParserMULTIPLY || _la == ExprParserDIVIDE) {
						...
						localctx.(*MulDivContext).mulOp = _ri
					} else {
			...
			case 2:
				...
				}
				{
					...
					localctx.(*AddSubContext).addOp = _lt
					...
					if !(_la == ExprParserADD || _la == ExprParserSUB) {
						...
						localctx.(*AddSubContext).addOp = _ri
					} else {
						p.GetErrorHandler().ReportMatch(p)
						p.Consume()
					}
